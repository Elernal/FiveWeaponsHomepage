(function(){const e=document.createElement("link").relList;if(e&&e.supports&&e.supports("modulepreload"))return;for(const a of document.querySelectorAll('link[rel="modulepreload"]'))o(a);new MutationObserver(a=>{for(const s of a)if(s.type==="childList")for(const i of s.addedNodes)i.tagName==="LINK"&&i.rel==="modulepreload"&&o(i)}).observe(document,{childList:!0,subtree:!0});function n(a){const s={};return a.integrity&&(s.integrity=a.integrity),a.referrerPolicy&&(s.referrerPolicy=a.referrerPolicy),a.crossOrigin==="use-credentials"?s.credentials="include":a.crossOrigin==="anonymous"?s.credentials="omit":s.credentials="same-origin",s}function o(a){if(a.ep)return;a.ep=!0;const s=n(a);fetch(a.href,s)}})();const l={title:"福建五兵团练营",subtitle:"HEMA 俱乐部",assets:{logo:{path:"images/logo-five-weapons.jpg",alt:"福建五兵团练营标识"},partnerLogo:{path:"images/logo-black-tigers.jpg",alt:"Black Tigers 标识"},hero:{path:"images/hero-hema-recruit.png",alt:"两名 HEMA 训练者在室内进行长剑对练"}},navigation:[{label:"训练内容",href:"#training"},{label:"时间安排",href:"#schedule"},{label:"安全说明",href:"#safety"},{label:"加入方式",href:"#join"}],hero:{eyebrow:"历史武术 · 现代训练 · 福建",title:"以安全训练，进入真实的欧洲历史武术",summary:"福建五兵团练营面向 HEMA 爱好者与新人，提供长剑、基础体能、技法拆解和安全对抗训练。",primaryCta:"加入训练群",secondaryCta:"预约体验",stats:[{value:"HEMA",label:"历史欧洲武术"},{value:"安全",label:"护具与规则先行"},{value:"新人",label:"零基础可体验"}]},intro:{heading:"不是表演，是可训练、可复盘的武术系统",body:"我们从历史文献、现代运动训练和实战安全规则出发，学习兵器距离、步法、攻防判断与对抗节奏。训练强调纪律、保护和持续进步。"},training:[{title:"长剑基础",text:"握持、站架、步法、切击线路、距离控制与基础攻防。"},{title:"技法拆解",text:"围绕历史技法进行分段练习，把动作还原为可执行的训练任务。"},{title:"安全对抗",text:"在护具、规则和教练监督下，逐步进入轻量到正式强度的对练。"},{title:"体能与协调",text:"补足核心、下肢、肩背和反应训练，让动作更稳定、更可控。"}],schedule:[{label:"常规训练",value:"每周训练时间待补"},{label:"新人体验",value:"可提前预约，具体场次待补"},{label:"训练地点",value:"福建地区训练点待补"}],strengths:["从安全规则和装备使用开始，降低新人入门压力。","训练目标清晰，每次练习都能对应到动作、距离或判断能力。","兼顾技术、体能和对抗，不把 HEMA 做成单纯摆拍。","保留社群交流空间，方便新人提问、约练和复盘。"],safety:{heading:"安全优先，循序进入对抗",points:["新人先进行基础动作、距离感和装备说明，不直接进入高强度对抗。","对练需遵守护具、武器、强度和停止口令要求。","训练中如有身体不适、装备问题或动作风险，应立即暂停。"]},join:{heading:"加入训练群",body:"真实联系方式暂未公开。请将微信号、训练群二维码、报名表或邮箱补充到此处后即可正式上线。",contacts:[{label:"微信 / 训练群",value:"待补：训练群二维码或联系人微信"},{label:"邮箱",value:"待补：club@example.com"},{label:"体验说明",value:"请提前预约，穿着便于运动的服装到场。"}]},footer:{brand:"福建五兵团练营",note:"Black Tigers 标识作为社群历史或关联标识预留展示。"}};function u(t,e){console.error(`[FiveWeaponsHomepage] ${t}`,e)}function h(t,e){console.info(`[FiveWeaponsHomepage] ${t}`,e)}const d={error:u,info:h};function g(t,e="/FiveWeaponsHomepage/"){return`${e}${t}`}function f(t,e=""){const n=g(t);return new Promise((o,a)=>{const s=new Image;s.alt=e,s.addEventListener("load",()=>{o(s)}),s.addEventListener("error",()=>{const i=new Error(`图片加载失败：${n}`);d.error("图片加载失败",{source:n,alt:e,error:i}),a(i)}),s.src=n})}const p=document.querySelector("#app");async function b(){try{const t=await v(l.assets);m(t)}catch(t){d.error("官网初始化失败",t),P()}}async function v(t){const e=Object.entries(t),n=await Promise.all(e.map(y));return Object.fromEntries(n)}async function y(t){const[e,n]=t,o=await f(n.path,n.alt);return[e,o]}function m(t){const e=l;p.innerHTML=`
        ...
        <main id="top">
            <section class="hero">
                ...
                <div class="hero-content">
                    ...
                    <dl class="hero-stats">${j(e.hero.stats)}</dl>
                </div>
            </section>
            ...
            >
                ...
                <div class="feature-grid">${w(e.training)}</div>
            </section>
            <section
            ...
            >
                ...
                <div class="schedule-list">${E(e.schedule)}</div>
            </section>
            <section class="section strength-section" aria-labelledby="strength-title">
                ...
                <ul class="strength-list">${c(e.strengths)}</ul>
            </section>
            <section
            ...
            >
                ...
                <ul class="safety-list">${c(e.safety.points)}</ul>
            </section>
            <section class="join-section" id="join" aria-labelledby="join-title">
                ...
                <div class="contact-panel">${L(e.join.contacts)}</div>
            </section>
        </main>
    ...
    `,H(t)}function $(t){return t.map(e=>`<a href="${e.href}">${e.label}</a>`).join("")}function j(t){return t.map(e=>`<div><dt>${e.value}</dt><dd>${e.label}</dd></div>`).join("")}function w(t){return t.map(e=>`<article><h3>${e.title}</h3><p>${e.text}</p></article>`).join("")}function E(t){return t.map(e=>`<div><span>${e.label}</span><strong>${e.value}</strong></div>`).join("")}function c(t){return t.map(e=>`<li>${e}</li>`).join("")}function L(t){return t.map(e=>`<div><span>${e.label}</span><strong>${e.value}</strong></div>`).join("")}function H(t){r(".brand-logo",t.logo,"site-logo"),r(".hero-image",t.hero,"hero-img"),r(".footer-logo",t.partnerLogo,"partner-logo")}function r(t,e,n){const o=document.querySelector(t),a=e.cloneNode();a.className=n,o.append(a)}function P(){p.innerHTML=`
        <main class="fallback">
            <h1>${l.title}</h1>
